...
*local VARIABLES
*
      integer eo,oe,mu,i,site,measflag,acc
...
*
      ...
      do 10 eo=0,1
        ...
        oe=mod(eo+1,2)
         ...
         do 20 mu=1,3
************************************************ 
*Pure gauge update. The uddate consist of 4 parts
************************************************

*
* 1) calculates the staple
*
            call staples(eo,oe,mu)
*
* 2) normalize the staple v to su(2) matrix u_tmp1, calculates 
*    1/2 Tr (u*u_tmp1)=tmp2 and calculates 1/(beta norm)=tmp1
*    and this is done for all sites i
*
     ...
     &           v(2,site,eo,mu)*conjg(v(2,site,eo,mu)))
            ...
            u_tmp1(1,site)=tmp1(site)*v(1,site,eo,mu)
            u_tmp1(2,site)=tmp1(site)*v(2,site,eo,mu)

            tmp1(site)=tmp1(site)/beta

            tmp2(site)=real(u(1,site,eo,mu)*u_tmp1(1,site)-
     &          u(2,site,eo,mu)*conjg(u_tmp1(2,site)))
           enddo
*
*3) create SU(2) matrices with probability distribution
*   exp{ 1/(2*a) Tr V },    with V = u_tmp2 and a = tmp1.
*   with Kennedy-Pendleton heatbath.
*   input : tmp1, tmp2
*   output : u_tmp2,acc
*

          call heat_kpi(tmp1,u_tmp2,tmp2,acc)
          accept=accept+real(acc)
* 4)
* new U's are:  U = u_tmp2 * u_tmp1^dagger
*
      call multallnd(u_tmp2(1,1),u_tmp1(1,1),u_tmp3(1,1),maxarray,
...
c      goto 9
****************************************************************
* Calculation of Gauge+Higgs interaction  and accept/reject step         
****************************************************************
